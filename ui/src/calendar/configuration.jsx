...
class Configuration extends React.PureComponent {
	...
	handlePdfWorkerMessage = ( { data: { blob, pageMap } } ) => {
	...
	handleDownloadDirect = () => {
		saveAs( this.state.pendingBlob, 'recalendar.pdf' );
		this.setState( { showProcessModal: false, pendingBlob: null } );
	};

	handleProcessPdf = async () => {
		const { pendingBlob, docName } = this.state;
		if ( ! docName.trim() ) return;
		this.setState( { isProcessing: true, processError: null } );
		const fd = new FormData();
		fd.append( 'pdf', pendingBlob, 'recalendar.pdf' );
		fd.append( 'name', docName.trim() );
		if ( this.state.pageMap ) {
			fd.append( 'pageMap', JSON.stringify( this.state.pageMap ) );
			fd.append( "topMargin", this.state.topMargin ? "true" : "false" );
		}
		try {
		...
		pageMap: null,
			...
			this.setState( {
				isProcessing: false,
				processError: String( err.message || err ),
			} );
		}
	};
	...
	handleDayItineraryAdd = ( event ) => {
		const newItineraries = [ ...this.state.dayItineraries ];
		const { field, type } = event.target.dataset;
		const items = [ ...newItineraries[ field ].items ];

		// Check if already added
		const alreadyHas = items.some( ( item ) => item.type === type );
		if ( alreadyHas ) return;

		if ( type === ITINERARY_TASKS || type === ITINERARY_NOTES ) {
			items.push( wrapWithId( { type: ITINERARY_NEW_PAGE, value: "" } ) );
			items.push( wrapWithId( { type, value: "" } ) );
			items.push( wrapWithId( { type: ITINERARY_LINES, value: 0 } ) );
		} else if ( type === ITINERARY_AGENDA ) {
			// Remove existing lines=0 items first
			const filtered = items.filter(
				( item ) => !( item.type === ITINERARY_LINES && ( item.value === 0 || item.value === "0" || item.value === "" ) ),
			);
			filtered.push( wrapWithId( { type, value: "" } ) );
			filtered.push( wrapWithId( { type: ITINERARY_LINES, value: 0 } ) );
			newItineraries[ field ] = { ...newItineraries[ field ], items: filtered };
			this.setState( { dayItineraries: newItineraries } );
			return;
		} else {
			items.push( wrapWithId( { type, value: "" } ) );
		...
		newItineraries[ field ] = { ...newItineraries[ field ], items };
		this.setState( { dayItineraries: newItineraries } );
	};
				...
				<Accordion defaultActiveKey="start" className="my-3">
					...
					<Accordion.Item eventKey="general">
						...
						<Accordion.Body>
							...
							<Form.Group controlId="topMargin" className="mt-2">
								<Form.Check
									label={ t( 'configuration.general.top-margin.label' ) }
									type="checkbox"
									checked={ this.state.topMargin }
									value={ this.state.topMargin }
									onChange={ this.handleFieldChange }
								/>
								<Form.Text className="text-muted">
									{t( 'configuration.general.top-margin.description' )}
								</Form.Text>
							</Form.Group>
	...
	render() {
		...
		return (
			<Container className="h-100" fluid>
				...
				<Modal
					show={ this.state.showProcessModal }
					onHide={ () => {} }
					centered
					backdrop="static"
				>
					<Modal.Header>
						<Modal.Title>{t( 'process.title' )}</Modal.Title>
					</Modal.Header>
					<Modal.Body>
						<div className="d-flex gap-2 mb-3">
							<Button variant="outline-primary" onClick={ this.handleDownloadDirect }>
								{t( 'process.download' )}
							</Button>
							<Button
								variant="outline-success"
								onClick={ () => this.setState( { wantProcess: true } ) }
								disabled={ this.state.wantProcess }
							>
								{t( 'process.upload' )}
							</Button>
						</div>
						{this.state.wantProcess && (
							<InputGroup className="mb-3">
								...
								<Button
									variant="success"
									disabled={ ! this.state.docName.trim() || this.state.isProcessing }
									onClick={ this.handleProcessPdf }
								>
									{this.state.isProcessing ? t( 'process.working' ) : t( 'process.confirm' )}
								</Button>
							</InputGroup>
						)}
						{this.state.processError && (
							<div className="text-danger mt-2 small">{this.state.processError}</div>
						)}
					</Modal.Body>
				</Modal>
</Container>
