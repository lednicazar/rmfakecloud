			...
			<Stack direction="horizontal" className="mt-3" gap={ 3 }>
				...
				<ButtonGroup>
					<Button
						...
						onClick={ onAdd }
						data-type={ ITINERARY_AGENDA }
						data-field={ field }
						disabled={ itinerary.some( ( item ) => item.type === ITINERARY_AGENDA ) }
					>
						{t( 'configuration.itinerary.button.agenda' )}
					</Button>
					<Button
						variant="outline-success"
						onClick={ onAdd }
						data-type={ ITINERARY_TASKS }
						data-field={ field }
						disabled={ itinerary.some( ( item ) => item.type === ITINERARY_TASKS ) }
					>
						{t( 'configuration.itinerary.button.tasks' )}
					</Button>
					<Button
						variant="outline-info"
						onClick={ onAdd }
						data-type={ ITINERARY_NOTES }
						data-field={ field }
						disabled={ itinerary.some( ( item ) => item.type === ITINERARY_NOTES ) }
					>
						{t( 'configuration.itinerary.button.notes' )}
					</Button>
				</ButtonGroup>
			</Stack>
			{onCopy && (
